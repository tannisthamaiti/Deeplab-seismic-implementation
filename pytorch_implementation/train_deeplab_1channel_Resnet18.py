...
from core.models import deeplab_1channel_Resnet18
import core.loss
...
def train(args):
    ...
    if args.resume is not None:
        ...
        #beta=args.beta)
        # edited by Tannistha
        model = getattr(deeplab_1channel_Resnet18, 'resnet18')(
    ...
    #print(list(model.module.conv1.parameters()) + list(model.module.bn1.parameters()) +
    #    list(model.module.layer1.parameters()) +
    #    list(model.module.layer2.parameters()) +
    #    list(model.module.layer3.parameters()) +
    #    list(model.module.layer4.parameters()))
    #print(model.train().module.conv1)
    # declaring it outside if statement by Tannistha
    backbone_params = (list(model.train().module.conv1.parameters()) + list(model.train().module.bn1.parameters()) +
        list(model.train().module.layer1.parameters()) +
        list(model.train().module.layer2.parameters()) +
        list(model.train().module.layer3.parameters()) +
        list(model.train().module.layer4.parameters()))
    last_params = list(model.train().module.aspp.parameters())
    ...
    model_fname = 'data/deeplab_resnet_18_lr_0.001_epoch%d.pth'.format(args.exp)
    # training
    pred_class = []
    pixel_acc = []
    class_acc = []
    ...
    for epoch in range(args.n_epoch):
        ...
        for i, (images, labels) in enumerate(trainloader):
            ...
            #edited by Tannistha tensor-> 1channel_image->RGB
            #image = transforms.ToPILImage(mode='L')(images).convert("RGB")
            #image = to_3_channels(images)
            ...
            #    print("Epoch {%d/%d}\t\t" 
            #          "iter: {1}/{2}\t\t" 
            #          "training Loss: %.4f".format(epoch + 1, i + 1, len(trainloader), loss.item()))
        ...
        if args.per_val != 0:
            with torch.no_grad():  # operations inside don't track history
                ...
                pixel_acc.append(running_metrics_val.get_scores()[0]["Pixel Acc: "])
                class_acc.append(running_metrics_val.get_scores()[0]["Class Accuracy: "])
                mean_class_acc.append(running_metrics_val.get_scores()[0]["Mean Class Acc: "])
                fre_weighted_iou.append(running_metrics_val.get_scores()[0]["Freq Weighted IoU: "])
                mean_iou.append(running_metrics_val.get_scores()[0]["Mean IoU: "])
                confusion_mat.append(running_metrics_val.get_scores()[0]["confusion_matrix"])
                cls_iu.append(running_metrics_val.get_scores()[1])
            ...
            if (epoch+1) % 5 == 0:
                ...
                torch.save({'epoch': epoch + 1,
                        'state_dict': model.state_dict(),
                        'optimizer': optimizer.state_dict(),}, model_fname % (epoch + 1))
    pd.DataFrame(pixel_acc).to_csv("val_lr_0.001/metrics/pixel_acc.csv", index=False)
    pd.DataFrame(class_acc).to_csv("val_lr_0.001/metrics/class_acc.csv", index=False)
    pd.DataFrame(mean_class_acc).to_csv("val_lr_0.001/metrics/mean_class_acc.csv",index=False)
    pd.DataFrame(fre_weighted_iou).to_csv("val_lr_0.001/metrics/freq_weighted_iou.csv",index=False)
    pd.DataFrame(mean_iou).to_csv("val_lr_0.001/metrics/mean_iou.csv", index=False)
    pd.DataFrame(cls_iu).to_csv("val_lr_0.001/metrics/cls_iu.csv", index=False)
    for i in range(len(confusion_mat)):
        name = "val_lr_0.001/metrics/confusion_matrix/confusion_matrix_" + str(i) + ".csv"
